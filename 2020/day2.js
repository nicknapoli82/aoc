...
input = input.map((s) => { let res = { range: s[0].split('-'), letter: s[1][0], password: s[2] }; return res; });
let result = 0;
for (let test of input) {
  let meetsPolicy = test.password.match(RegExp(test.letter, 'g'));
  if (meetsPolicy != null && test.range[0] <= meetsPolicy.length && test.range[1] >= meetsPolicy.length)
    result++;
}
...
result = 0;
for (let test of input) {
  let meetsNewPolicy1 = test.password[test.range[0] - 1] == test.letter;
  let meetsNewPolicy2 = test.password[test.range[1] - 1] == test.letter;
  if ((meetsNewPolicy1 && !meetsNewPolicy2) || (!meetsNewPolicy1 && meetsNewPolicy2))
    result++;
}
